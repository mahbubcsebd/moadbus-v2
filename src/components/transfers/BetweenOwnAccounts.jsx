...
export default function BetweenOwnAccounts({ onSubmit, isSubmitting = false }) {
  ...
  const {
    ...
    defaultValues: {
      fromAccount: accountFromQuery || '', // Set default from query parameter
      toAccount: '',
      amount: '',
      description: '',
    ...
    () =>
      accounts
        ...
        .map((acc) => ({
          ...
          balance: acc.balance,
          currency: acc.currency || 'USD', // Default to USD if not specified
        })),
    [accounts],
  );
  ...
  const currency = selectedFromAccount?.currency || 'USD';

  // Filter out the selected from account from the to account options
  const toAccountOptions = useMemo(
    () => accountOptions.filter((opt) => opt.value !== fromAccount),
    [accountOptions, fromAccount],
  );

  // Set the default from account when accounts are loaded
  useEffect(() => {
    if (accountFromQuery && accountOptions.length > 0) {
      // Check if the account from query exists in our options
      const accountExists = accountOptions.some((opt) => opt.value === accountFromQuery);
      if (accountExists) {
        setValue('fromAccount', accountFromQuery);
      }
    }
  }, [accountFromQuery, accountOptions, setValue]);
  ...
  const handleReset = () => {
    reset({
      fromAccount: accountFromQuery || '', // Reset to default from query parameter
      toAccount: '',
      amount: '',
      description: '',
      transferType: 'immediate',
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (res.rs.status === 'success') {
        showSuccess(res.rs, 'Transfer Between My Accounts At Moadbus');
        reset();
        refresh();
      } else {
  ...
  return (
    <>
      <div>
        <HeaderTop
          title="Transfer Between My Accounts At Moadbus Bank"
