...
export default function GlobalSuccess() {
  ...
  if (Array.isArray(successData)) {
    receiptItems = successData;
  } else if (successData.rcpt || successData.Rcpt) {
    const rawRcpt = successData.rcpt || successData.Rcpt;
    receiptItems = rawRcpt
      .split('|')
      ...
      .map((item) => {
        ...
        if (label && label.toLowerCase().includes('description')) {
          try {
            ...
            console.error('Decoding error for description field:', e);
          }
        }
        return { label, value };
      });
  ...
  return (
    <Dialog open={isOpen} onOpenChange={hideSuccess}>
      <DialogContent className="max-w-[calc(100%-2rem)] sm:max-w-md w-full p-0 gap-0 max-h-[90vh] overflow-y-auto z-[9999999999]">
          ...
          <div className="mb-6 space-y-3 text-sm font-semibold text-gray-700">
            {receiptItems.length > 0 ? (
              receiptItems.map((row, idx) => (
                <div
                  key={idx}
                  className="flex justify-between gap-10 pb-1 border-b border-gray-50 last:border-0"
                >
                  <span className="text-sm text-left text-gray-500 whitespace-nowrap">
                    {row.label}
                  </span>
                  <span className="text-sm text-right text-gray-900 wrap-break-word">
                    {row.value}
                  </span>
                </div>
              ))
